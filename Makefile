...
.PHONY: prevalidate
prevalidate: ## The pre-brief gate, Docker-free (~3 min): profile preflight + closed-loop sim + hot-path benches. The kind/dev-50 rung runs DEVPOD-SIDE as every brief's fail-fast step 0 (bigfleet-uber protocol v1.2); `make prevalidate-kind` keeps it runnable locally for harness diagnostics.
	...
	@echo "[$$(date +%T)] rung 1: closed-loop sim"
	$(GO) test -count=1 -run ClosedLoop ./sim/...
	@echo "[$$(date +%T)] rung 2: hot-path benches"
	$(MAKE) bench-hot
	@echo "[$$(date +%T)] prevalidate green — SHA is brief-ready (the kind rung runs devpod-side as brief step 0)"

.PHONY: prevalidate-kind
prevalidate-kind: prevalidate ## prevalidate + dev-50 on kind (~8 min warm, needs Docker). On-demand local variant of the rung the devpods run per brief — use it when working ON the harness.
	@docker info >/dev/null 2>&1 || { echo "prevalidate-kind: Docker daemon not running — start Docker Desktop first"; exit 1; }
	@echo "[$$(date +%T)] kind rung: images + kind"
...
vet: ## Run go vet, including every build-tagged test package — tagged code rotted invisibly twice (integration, scale) when nothing compiled it.
	...
	$(GO) vet -tags=scale ./test/scale/...
	$(GO) vet -tags=conformance ./test/conformance/...
