...
- **A node should count as capacity only once it has actually joined.** A node's value is realised when kubelet has registered and the node is `Ready` on its target cluster's network — not when its VM boots. Providers must not report a machine `Configured` before the node is genuinely ready, or BigFleet can credit capacity that isn't schedulable. Tightening this guarantee inside BigFleet itself — gating coverage credit on observed node readiness rather than provider-asserted state — is under active design review (ADR-0056).
