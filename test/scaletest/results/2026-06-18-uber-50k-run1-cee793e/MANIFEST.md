# uber-50k reproducibility receipt #1 — FAITHFUL cee793e (#88)
...
`2026-06-18-uber-50k-run1-cee793e`.
...
|---|---|---|---|
| shardConfigurePhaseP99 | <=15s | **1.20s** | 1.21s |
| shardCycleDurationP99 | <=25s (50k) | **4.08s** | 4.08s |
| bootstrapSuccessRatio | >=0.99 | **1.000** | 1.0 |
| operatorNodeStateUpdateP99 | <=1.5s | **0.89s** | 0.873s |
| operatorRollupP99 | <=1s | **0.92s** | 0.757s |
| operatorAckP99 | <=12s | **1.12s** | 1.28s |
| shardShortfalls | ==0 | **0** | 0 |
| aggregateInventory | (canonical 225,206) | **225202** | 225,206 |
...
## Artifacts
`tsdb-snapshot.tar.gz` (RELEASE ASSET, see `https://github.com/intUnderflow/bigfleet/releases/download/scaletest-receipts-uber50k-20260621/tsdb-uber-50k-run1-cee793e.tar.gz`): hub + one-sat-per-region TSDB sample.
