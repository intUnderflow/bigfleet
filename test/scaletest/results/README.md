...
| homelab-500k | — | — | — | — | — | — | — | **partially unblocked** — M11.15 added an `(state, instance-type)` index in `pkg/inventory` + Phase 1 candidate-pool narrowing. Bench shows ~2× cycle improvement (80 s → 37 s at 50K) but still O(N×M) over the matched-type bucket. The M11.16 follow-up (per-Profile pool caching with cross-Profile claim coordination) is the next algorithmic step before the cloud 500K test is worthwhile. |
