...
| **scaleway-500k** | [20260504-091949-scaleway-500k-tmpfs](20260504-091949-scaleway-500k-tmpfs/) | **0.062 s** | 830 ms | 380 ms | 50,000 / 500,000 | 50 | **✓ PASS** | First passing run. Tmpfs-backed shared volume between the apiserver and workload containers — kine sqlite WAL fsyncs land in RAM (~µs) instead of node disk (~ms). Cycle p99 0.156 → 0.062 s (−60 % runner-side, matches the steady-state replay we already had). Operator ack p99 dropped 4.64 s → 380 ms. **Cumulative trajectory M11.18 → here: 4.08 s → 62 ms (−98.5 %).** rollup p99 (830 ms) and the `loadgenCRsActive` (13,464 vs target 50,000) are still harness-side ceilings (the kwok pod's apiserver QPS is the next harness lever) but neither is gated by the runner's SLO check. |
| scaleway-500k (snap.All() removal) | [20260504-083940-scaleway-500k-snapall](20260504-083940-scaleway-500k-snapall/) | 0.156 s | 16 ms | 4.64 s | 50,000 / 500,000 | 50 | ✗ runner / ✓ replay | Killed snap.All() walk + Shortfalls() alloc on the cycle hot path. Replay 62 ms; runner 156 ms (rate window caught early-soak transients). Operator ack 4.64 s — kine WAL on disk was the bottleneck. |
| scaleway-500k (fake-idx) | [20260504-001519-scaleway-500k-fakeidx](20260504-001519-scaleway-500k-fakeidx/) | 0.24 s | 26 ms | 278 ms | 50,000 / 500,000 | 50 | ✗ | Fake-provider lastModRev index + RWMutex + workload container CPU bump. Cycle p99 0.51 → 0.24 s (−53 %); reconcile p99 collapsed 508 ms → <1 ms. |
| scaleway-500k (warmup + split) | [20260503-233227-scaleway-500k-warmup-split](20260503-233227-scaleway-500k-warmup-split/) | 0.51 s | 16 ms | 475 ms | 50,000 / 500,000 | 50 | ✗ | Metrics warmup + harness pod-split. Execute p99 604 → 2.6 ms (−230×); cycle didn't move because reconcile (508 ms p99) was hiding behind execute. |
| scaleway-500k (M11.24a + AC + topology) | [20260503-224604-scaleway-500k-m1124a](20260503-224604-scaleway-500k-m1124a/) | 0.51 s | 16 ms | 391 ms | 50,000 / 500,000 | 50 | ✗ | M11.24a + AvailableCapacity surfacing + topology Same/Spread. Cycle p99 0.63 → 0.51 s (−20 %). |
| scaleway-500k (M11.23) | [20260502-124541-scaleway-500k-m1123](20260502-124541-scaleway-500k-m1123/) | 0.63 s | 16 ms | 504 ms | 50,000 / 500,000 | 50 | ✗ | M11.23 Phase 3 fingerprint grouping. Cycle p99 0.71 s → 0.63 s (−11 %). |
