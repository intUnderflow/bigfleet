...
| **scaleway-500k** | [20260503-224604-scaleway-500k-m1124a](20260503-224604-scaleway-500k-m1124a/) | **0.51 s** | 16 ms | 391 ms | 50,000 demand / 500,000 inventory | 50 | ✗ | M11.24a (drop reconcile proto round-trip + early state-match short-circuit) + AvailableCapacity surfacing (paper §6.2) + topology Same/Spread enforcement (paper §8). Cloud cycle p99 dropped 0.63 s → 0.51 s (−20 %); ack p99 504 → 391 ms (−22 %). First run of this commit set pinned the kwok pods at their 1-core CPU limit because AvailableCapacityUpdate emitted every cycle (~10 Hz) → ~500 CR upserts/sec into the operator-side kine sqlite; rate-limit + coalesce (default 5s per (cluster, fingerprint)) brought the harness back to a normal CPU profile. Demand fully satisfied (`shortfalls=0`). Per-phase p99s came back as -1 — the prom queries returned empty; needs a separate diagnosis of the runner's query timing vs scrape interval. |
