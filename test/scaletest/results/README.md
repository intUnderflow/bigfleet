...
| **scaleway-500k** | [20260501-190456-scaleway-500k](20260501-190456-scaleway-500k/) | **4.03 s** | 18 ms | 513 ms | 50,000 demand / 500,000 inventory | 50 | ✗ | First passing 500K-machine real-cluster run for everything except the shard cycle SLO. Operator paths comfortably under SLO; shard cycle 40× over the 100 ms threshold. The 4 s is dominated by the cycle's *execute* phase (50K Bootstrap actions, each calling provider.Configure + walking the inventory state machine), not Phase 1's decision work which the M11.16 bench predicted at ~2.15 s. The harness's "demand arrives all at once" pattern is a worst case; real production would see hundreds of actions per cycle, not 50K. Cost: ~$0.95 on a 5× PRO2-M Scaleway nl-ams cluster. |
