...
| **scaleway-500k** | [20260502-104945-scaleway-500k-m1120](20260502-104945-scaleway-500k-m1120/) | **3.69 s** | 16 ms | 459 ms | 50,000 demand / 500,000 inventory | 50 | ✗ | M11.19 (incremental snapshots) + M11.20 (cost-ordered cursor with lazy MatchProfile). Single shard cycle p99 dropped 4.08 s → 3.69 s (−10 %). Demand fully satisfied (`shortfalls=0`); ack p99 also improved 542 ms → 459 ms (−15 %). **Smaller cloud delta than the M5 bench projection (~50 %)** — bench at 500K showed 1.68 s → 0.99 s, which on PRO2 single-thread should project to ~2 s, not 3.7 s. The gap (≈1.7 s residual) is something the bench doesn't capture: reconcile, allocator interactions with Configured-state growth at steady state, GC, or PRO2 memory bandwidth. Diagnosis pending. |
