...
| **scaleway-500k** | [20260504-083940-scaleway-500k-snapall](20260504-083940-scaleway-500k-snapall/) | **0.156 s** runner / **0.063 s** end-of-soak replay | 16 ms | 4.64 s | 50,000 / 500,000 (loadgen 37,329 active) | 50 | ✗ runner / **✓ steady-state** | Killed `snap.All()` walk on the cycle hot path (replaced 500K slice copy + per-machine state-count loop with 9 O(1) `CountByState` reads) and the wasted alloc in the shortfall-gauge update. Shard CPU dropped from 1.26 cores → 402 m. **End-of-soak snapshot replay shows cycle p99 = 62.5 ms — under the 100 ms SLO.** The runner's 156 ms is its 5-min rate window catching early-soak transients; if the soak ran longer, the window would slide forward and the runner-side number would converge to the steady-state replay. Phase 3 (62 ms p99) is now essentially the entire cycle. The non-phase gap from prior runs is gone. Operator ack p99 4.64 s and rollup 900 ms: load-driver now creates at 155/sec (vs 86/sec last run), apiserver ack write-rate can't keep up — that's harness, not shard. |
