...
| **scaleway-500k** | [20260502-115342-scaleway-500k-m1122](20260502-115342-scaleway-500k-m1122/) | **0.71 s** | 19 ms | 496 ms | 50,000 demand / 500,000 inventory | 50 | ✗ | M11.22 incremental reconcile via `since_revision`. **Shard cycle p99 dropped 3.69 s → 0.71 s (−81 %)** — bigger than the M5 phase-dump projection (which estimated reconcile alone would drop from 696 ms → 73 ms). Cloud reaped the same delta on PRO2-M plus secondary wins (less GC pressure, less memory bandwidth pressure from per-cycle 500K proto round-trips). Phase 3 (reclaim) is now the dominant cost (M5 phase-dump: 131 ms mean) and is the next milestone target. Demand fully satisfied (`shortfalls=0`). Still over the 100 ms cycle SLO; trajectory is clear. |
