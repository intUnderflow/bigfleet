...
| **scaleway-500k** | [20260503-233227-scaleway-500k-warmup-split](20260503-233227-scaleway-500k-warmup-split/) | **0.51 s** | 16 ms | 475 ms | 50,000 / 500,000 (loadgen reached only 14,755 active) | 50 | ✗ | Metrics warmup (5 cycles) + harness pod-split (apiserver and workload in separate containers, separate cgroups). Snapshot replay shows the **harness split worked spectacularly: execute p99 604 ms → 2.6 ms (−230×).** The kine apiserver contention was the entire execute tail. Cycle p99 didn't move because the new dominant phase is **reconcile (508 ms p99)** — the fake provider's `List` filter walks all 500K machines per cycle even with `SinceRevision` set, and its single Mutex serialises with the shard's parallel Configure RPCs (executeConcurrency=128). Both are fake-provider-implementation artifacts, not shard issues. Workload container at 500m CPU limit may also be throttled — `loadgenCRsActive` only hit 14,755 vs 50,000 target. Next: index the fake provider's `lastModRev` for O(delta) List, switch its Mutex to RWMutex, bump workload container CPU. |
