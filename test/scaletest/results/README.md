...
| **scaleway-500k** | [20260504-001519-scaleway-500k-fakeidx](20260504-001519-scaleway-500k-fakeidx/) | **0.24 s** | 26 ms | 278 ms | 50,000 / 500,000 (loadgen 17,586 active) | 50 | ✗ | Fake-provider lastModRev index + RWMutex + workload container CPU 500m → 1.5 cores. **Cycle p99 dropped 0.51 s → 0.24 s (−53 %).** Snapshot replay: reconcile p99 508 ms → <1 ms (the index is O(delta), no longer O(N=500K)). Phase 3 (60 ms p99) is the new dominant identifiable phase; cycle-total minus sum-of-phases is ~160 ms of non-phase overhead worth investigating. Demand fully satisfied (`shortfalls=0`). Still ~2.4× over the 100 ms cycle SLO; loadgenCRsActive still below target — workload container may want more headroom or the harness still has tail effects we haven't traced. |
