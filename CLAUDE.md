...
- **Demand realism before mechanism (ADR-0043).** Any engine mechanism motivated by harness-observed evidence must first answer: would a production fleet emit the demand shape that triggers this? If not, fix the harness and re-measure before designing mechanism. The ADR-0042 parking layer is the cautionary tale — built rigorously against demand one catalog archetype fabricated.
...
- **Climb the validation ladder; never cloud-first.** A cloud scale run is the last confirmation of a change, not the discovery instrument. The ladder (docs/scaletest.md §"The validation ladder"): `make prevalidate` (Docker-free, ~3 min — profile preflight + closed-loop sim + hot-path benches; decision-engine feedback bugs, per-cycle cost, seed/demand arithmetic) → the kind/dev-50 integration rung (harness wiring; runs DEVPOD-SIDE as step 0 of every cloud brief, fail-fast — `make prevalidate-kind` for on-demand local runs when working on the harness) → cloud. Every SHA bound for a cloud brief passes `make prevalidate` before the brief is filed; the brief executor fail-fasts on its own integration rung before spending a cloud profile. A cloud failure a lower rung would have caught is a process bug. Don't run the kind rung on the laptop as a routine gate — it burns the dev box for work the devpods do free.
