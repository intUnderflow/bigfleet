...
- **Climb the validation ladder; never cloud-first.** A cloud scale run is the last confirmation of a change, not the discovery instrument. The ladder (docs/scaletest.md §"The validation ladder"): closed-loop sim (`go test -run ClosedLoop ./sim/...`, seconds — decision-engine feedback bugs) → `make bench-hot` (~1 min — per-cycle cost at measured uber-5k cardinality) → `make prevalidate` (~10 min — dev-50 on kind, harness wiring) → cloud. Every SHA bound for a cloud brief passes `make prevalidate` first; a cloud failure that a lower rung would have caught is a process bug.
